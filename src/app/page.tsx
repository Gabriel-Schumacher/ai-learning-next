        ...
        <div className="w-full h-full">
          {!data.errorMessage && (
            <>
              ...
              {data.sortedData?.currentPage === "HOME" && <AiMenu />}
              {/* When Chatting */}
              {data.sortedData?.currentPage === "CHAT" && <AiChat />}
              {/* Data Creation */}
              ...
              {/* Quiz */}
              {
                data.sortedData?.currentPage === "QUIZ" && <Quiz />
                /*
              ...
              {/*WritingAid/Essay */}
              {data.sortedData?.currentPage === "ESSAY" && <WritingAid />}
              {/* Library */}
              {data.sortedData?.currentPage === "LIBRARY" && <LibraryPage />}
              {/* Settings */}
              {/* Error when no page is set. */}
              {!data.sortedData?.currentPage && (
                <div className="w-full h-full grid place-items-center">
                  <div>
                    <p className="bg-error-500 rounded py-1 px-2 text-surface-950-50 font-bold mb-2">
                      Sorry, there was an error:
                    </p>
                    <p className="text-surface-900 dark:text-surface-50 text-center">
                      No Page Set
                    </p>
                  </div>
                </div>
